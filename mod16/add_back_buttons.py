...
for path in html_files:
    ...
    if "Back to Launcher" not in content and "<body" in content.lower():
        # Inject right after opening <body> tag
        import re
        content = re.sub(r'(<body[^>]*>)', r'\1\n' + back_button_html, content, count=1, flags=re.IGNORECASE)
        
        with open(path, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"Injected back button into: {path}")
    else:
         print(f"Skipped {path}")

print("Done injecting back buttons.")
